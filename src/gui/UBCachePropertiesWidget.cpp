  ...
  , mOtherSliderUsed(false)
{
    ...
    mKeepAspectRatio = true;// UBSettings::settings()->cacheKeepAspectRatio->get().toBool();
    ...
    mpModeComboBox->setCurrentIndex(1);
...
bool UBCachePropertiesWidget::CachesIsEmpty()
{
    return mCaches.empty();
}

void UBCachePropertiesWidget::CloseCaches()
{
    if(!mCaches.empty())
    {
